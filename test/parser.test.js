...
const syntaxChecks = [
  ...
  ["function with two params", "block f(x: int, y: boolean): "],
  ["function with no params + return type", "block f() sends int: "],
  // ["function types in params", "block f(g: (int) sends boolean): "],
  ["function types returned", "block f(): (int)->(int)->void: "],
  ["array type for param", "block f(x: [[[boolean]]]): "],
  ["array type returned", "block f(): [[int]]: "],
  ...
  ["short if", "if true: say(1)"],
  ["longer if", "if true: say(1) else: say(1)"],
  ["even longer if", "if true: say(1) else if false: say(1)"],
  ["while with empty block", "while true:"],
  ["while with one statement block", "while true: int x = 1"],
  ["repeat with long block", "stack 2: say(1) \nsay(2) say(3)"],
  ["if inside loop", "stack 3: if true: say(1)"],
  ...
  ["length", "send #c send #[1,2,3]"],
  ["boolean literals", "boolean x = false || true"],
  ["all numeric literal forms", "say(8 * 89.123 * 1.3E5 * 1.3E+5 * 1.3E-5)"],
  ["empty array literal", "say([int]())"],
  ["nonempty array literal", "say([1, 2, 3])"],
  ["some operator", "send some dog"],
  ["no operator", "send no dog"],
  ["random operator", "send random [1, 2, 3]"],
...
const syntaxErrors = [
  ...
  ["string lit with newline", 'say("ab\\zcdef")', /col 9/],
