...
const semanticChecks = [
  ...
  ["short return", ["block f():", "  say(1)"].join("\n")],
  ["long return", ["block f() sends bool:", "  send true"].join("\n")],
  ...
  //   ].join('\n'),
  // ],
  ["long if", ["if true:", "  say(1)", "else:", "  say(3)"].join("\n")],
  [
    "elseif",
    [
      "if true:",
  ...
  ["for over collection", ["for i in [2,3,5]:", "  say(1)"].join("\n")],
  ["for in range", ["for i in 1..<10:", "  say(0)"].join("\n")],
  ["repeat", ["stack 3:", "  int a = 1", "  say(a)"].join("\n")],
  ...
  ["outer variable", ["int x = 1", "while(false):", "  say(x)"].join("\n")],
  ...
  [
    ...
    ["block square(int x) sends int:", "  send x * x", "4 |> square"].join(
      "\n"
    ),
  ],
  [
...
const semanticErrors = [
  ...
  [
    ...
    ["block f() sends int:", "  send"].join("\n"),
    /int should be returned/,
  ],
  [
    "return type mismatch",
    ["block f() sends int:", "  send false"].join("\n"),
    /bool to a int/,
  ],
  [
  ...
  ["non-integer repeat", 'stack "1":', /Expected a letter or a digit/],
  ...
  [
    "shadowing",
    ["int x = 1", "while true:", "  int x = 1"].join("\n"),
    /Identifier x already declared/,
  ],
  ["call of uncallable", "int x = 1 say(x())", /Call of non-function/],
  [
    "Too many args",
    ["block f(int x):", "  f(1, 2)"].join("\n"),
    /1 argument\(s\) required but 2 passed/,
  ],
  [
    "Too few args",
    ["block f(int x):", "  say(1) f()"].join("\n"),
    /1 argument\(s\) required but 0 passed/,
  ],
  [
    "Parameter type mismatch",
    ["block f(int x):", "  say(1)", "f(false)"].join("\n"),
    /Cannot assign a bool to a int/,
  ],
  [
    "bad param type in fn assign",
    ["block f(int x):", "  send block g(float y):", "send f = g"].join("\n"),
  ],
  ...
  [
    "multiple dedents",
    ["int x = 1", "  int y = 2", "    int z = 3", "int a = 4"].join("\n"),
